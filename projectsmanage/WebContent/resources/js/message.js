...
var getMyContacts = function () {
	...
	contactSource = new EventSource("chat.web?action=getcontacts");
	contactSource.onmessage = function(event) {
		var contacts = JSON.parse(event.data);
		for ( var i in contacts) {
			var html = new Array();
			var unchecked = contacts[i].unchecked;
			var contactID = contacts[i].contactid;
			var contactName = contacts[i].contactname;
			var im = contacts[i].im;
			var li = $("#messageDialog .contacts ul li[id='contactID_"
					+ contactID + "']");
			if (!li || li.length <= 0) {
				html.push("<li id='" + "contactID_" + contactID + "''>");
				html.push("<a href='#' onclick='active(this); getMessages("
						+ contactID + ");'>");
				html.push(contactName);
				if (unchecked > 0 && im == "receiver") {
					html.push("<span class='badge'>");
					html.push(unchecked);
					html.push("</span>");
				}
				html.push("</a></li>");
				$("#messageDialog .contacts ul").prepend(html.join(""));
			} else {
				var span = $(li).find("span:first");
				if (!span || span.length <= 0) {
					if (unchecked > 0 && im == "receiver") {
						$(li).find("a:first").append(
								"<span class='badge'>" + unchecked
										+ "</span>");
					}
				} else {
					if (im == "receiver") {
...
var getMessages = function (contactID) {
	...
	messageSource = new EventSource(
			"chat.web?action=getmessages&contactid=" + contactID);
...
var getAllContacts = function() {
	$.ajax({
		...
		for(var i in contacts) {
			...
			if (!li || li.length <= 0) {
				html.push("<input type=\"button\" class=\"btn btn-default btn-sm contactbtn\" key=\"" + contact.id + "\" value=\"" + contact.realname +"\" onClick=\"contactBtnActive(this);\" >");
			}
		}
	...
	$("#newContactDialog").dialog({
		...
		buttons : [ {
			...
			click : function(){
				...
				$("#messageDialog .contacts ul li a:first").click();
				$(this).dialog("close");
				$("#newMessage").focus();
			}
		}, {
...
(function($) {
	var init = function (select, options) {
		...
		html.push("消息");
		html.push("<span class=\"badge\" id=\"msgcount\" style=\"display: none;\">");
		html.push("0");
		html.push("</span>");
		html.push("</a>");
		...
		html.push("        <a href=\"#\" onclick=\"active(this); showAllContacts();\">新建会话</a>");
		html.push("      </li>");
		html.push("    </ul>");
		html.push("  </div>");
		...
		html.push("  </div>");
		html.push("  <textarea id=\"newMessage\" onkeypress=\"if (event.keyCode==13) { sendMessage(); }\"></textarea>");
		html.push("</div>");
		...
		html.push("      <button class=\"btn btn-default\" type=\"button\" onClick=\"searchContact();\">");
		html.push("        查找");
		html.push("      </button>");
		html.push("    </span>");
		html.push("  </div>");
	...
	var doLink = function () {
		linkSource = new EventSource("chat.web?action=msgcount");
		linkSource.onmessage = function(json) {
			var count = json.data;
			if (count > 0) {
				$("#msgcount").show();
				$("#msgcount").text(count);
