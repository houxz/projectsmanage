...
@Controller
@RequestMapping("/modify11111111.web")
public class ModifyCtrl {
	...
	private TaskModel getNextModifyTask(Integer userid) {
		...
		try {
			...
			if (myProjects != null && !myProjects.isEmpty()) {
				...
				task = taskModelClient.selectMyNextModifyTask(_myProjectIDs, userid);
			}
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
		}
	...
	private TaskModel getNextModifyTaskByProjectId(Integer userid,Long projectid,Long taskid) {
		...
		try {
			...
			task = taskModelClient.selectMyNextModifyTaskByProjectId(projectid,taskid, userid);
			if(task != null && task.getId() != null)
