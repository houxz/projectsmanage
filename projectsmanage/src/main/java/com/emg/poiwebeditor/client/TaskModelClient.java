...
public class TaskModelClient {
	 ...
	 * @throws Exception
	 */
	public List selectUserCheckTask(List<Long> projectIDs, Integer userid, long num) throws Exception {
		List tasks =  null;
		try {
			...
			sql2.append(" with");
			sql2.append(" sorttable(projectid, sortvalue) as (values"); 
			for (int i = 0; i < projectIDs.size(); i++) {
				sql2.append("(").append(projectIDs.get(i)).append(",").append(i).append(")").append(",");
			}
			sql2.deleteCharAt(sql2.length() - 1);
			sql2.append(")");
			sql2.append(" update tb_task set starttime=now(),operatetime=now(),state=1,process=7,checkid=  ").append(userid);
			sql2.append(" from ( ");
			sql2.append(" select id from tb_task t ");
			sql2.append(" join sorttable p on p.projectid = t.projectid ");
			sql2.append(" where (state=1 and process=7 ) and t.projectid=p.projectid and checkid=").append(userid).append(" and tasktype=17002 order by p.sortvalue, t.id limit ").append(num).append(" ) as b(id) where tb_task.id = b.id returning tb_task.* ");
	...
	public List selectUserInitTask(List<Long> projectIDs, Integer userid, long num) throws Exception {
		...
		try {
			...
			sql2.append(" select id from tb_task t ");
			sql2.append(" join sorttable p on p.projectid = t.projectid ");
			sql2.append(" where (state=0 and process=0 ) and t.projectid=p.projectid and ( editid=0 or editid ISNULL) and tasktype=17001  order by p.sortvalue, t.id limit ");
	 ...
	 * @throws Exception
	 */
	public List selectUserInitCheckTask(List<Long> projectIDs, Integer userid, long num) throws Exception {
		List tasks =  null;
		try {
			StringBuffer sql2 = new StringBuffer();
			sql2.append(" with");
			...
			sql2.append(" select id from tb_task t ");
			sql2.append(" join sorttable p on p.projectid = t.projectid ");
			sql2.append(" where (state=0 and process=0 ) and t.projectid=p.projectid and ( checkid=0 or checkid ISNULL) and tasktype=17002  order by p.sortvalue, t.id limit ");
			sql2.append(num).append(" ) as b(id) where tb_task.id = b.id returning tb_task.* ");
